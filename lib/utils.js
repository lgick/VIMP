  ...
  goBot: function (bots, mapWidth, mapHeight) {
    ...
    for (i in bots) {
      if (bots.hasOwnProperty(i)) {
        r = bots[i].rotation;
        vX = Math.sin(r * (Math.PI / -180)) * 32;
        vY = Math.cos(r * (Math.PI / -180)) * 32;
        bots[i].x += Math.round(vX);
        bots[i].y += Math.round(vY);
       // bots[i].rotation = this.rangeNumber(r + 4, true);
        bots[i].x = this.rangeNumber(bots[i].x, true, mapWidth, 0);
        bots[i].y = this.rangeNumber(bots[i].y, true, mapHeight, 0);
      }
    }
